...
function getPullNumberFromSubject(subject: string): number | null {
...
async function getPullURLs(ref: string): Promise<string[]> {
  ...
  const makeURL = (num: number) => `https://github.com/electron/electron/pull/${num}`;
...
async function doOpen(opts: { object: string; print: boolean }) {
