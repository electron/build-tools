...
function findNoteInPRBody(body: string): string | null {
...
async function getPullRequestInfo(pullNumber: number) {
  let notes = null;
  let title = null;
...
function guessPRTarget(config: EVMBaseElectronConfiguration): string | null {
  ...
  console.warn(
    `Unable to guess default target PR branch -- generated version '${version}' should include 'nightly' or match V-x-y`,
  );

  return null;
}

function guessPRSource(config: EVMBaseElectronConfiguration): string {
...
program
  ...
  .action(async (options) => {
    const source = options.source || guessPRSource(evmConfig.current());
    const target = options.target || guessPRTarget(evmConfig.current());
...
“${pullRequest.title} (#${pullRequest.number})” by ${pullRequest.user?.login || 'unknown'}
...
Proceed?`,
      ...
      try {
        ...
        const platformExecutables: Partial<Record<NodeJS.Platform, string>> = {
          win32: 'electron.exe',
          darwin: 'Electron.app/',
          linux: 'electron',
        };
