...
import { depotPath, depotOpts } from './utils/depot-tools.js';
import { EVMBaseElectronConfiguration } from './evm-config.schema.js';

function gitStatus(config: EVMBaseElectronConfiguration): string {
  const exec = 'git';
  ...
  for (const args of switches) {
    try {
      outs.push(childProcess.execFileSync(exec, args, opts) as string);
    } catch {}
  }
  return outs
    .map((out) => out.trim())
...
program
  ...
  .action(() => {
    ...
    try {
      const names = evmConfig.possibleNames();
      if (names.length === 0) {
...
program
  ...
  .action((options) => {
    try {
      const { env } = depotOpts(evmConfig.current());
      ...
      if (options.json) {
        ...
        const logger = ([key, val]: [string, string | undefined]) =>
          console.log(`${exportKeyword} ${key}=${val}`);
