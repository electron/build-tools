...
function gitStatus(config: SanitizedConfig): string {
  ...
  for (const args of switches) {
    try {
      ...
      // ignore failures from individual git probes
    }
  }
  return outs
    .map((out) => out.trim())
...
program
  ...
  .action((options: { name: boolean; git: boolean; filepath: boolean }) => {
    try {
...
program
  ...
  .action(() => console.log(depot.DEPOT_TOOLS_DIR));
  ...
  .action((options: { json?: boolean }) => {
    try {
      ...
      if (options.json) {
        ...
        const logger = ([key, val]: [string, string | undefined]): void =>
...
program
  ...
  .action((name: string | undefined) => {
    try {
      ...
      const dir = name ?? 'electron';
      console.log(color.path(path.resolve(root, 'src', dir)));
    } catch (e) {
      fatal(e);
    }
  });
