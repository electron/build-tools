...
const getPayload = (html: string, start: string, end: string) =>
  html.substring(html.indexOf(start) + start.length, html.indexOf(end));

async function getXsrfToken(osid: string) {
...
async function getBugInfo(bugNr: number) {
  const profile = process.env.CHROME_SECURITY_PROFILE ?? 'Profile 1';
  // @ts-expect-error
  const chrome = await import('@marshallofsound/chrome-cookies-secure');
...
type RawIssue = [[never, any[]]];

function parseCveFromIssue(issue: RawIssue): string | null {
  const CVE_ID = 1223136;

  const issueData = issue[0][1];
  const issueMetaData = issueData[issueData.length - 1];
  const cveData = issueMetaData[2][14].find((d: number[]) => d[0] === CVE_ID);
  const cve = cveData[cveData.length - 2];

  return /\d{4}-\d{4,7}/.test(cve) ? `CVE-${cve}` : null;
}

export async function getCveForBugNr(bugNr: number) {
