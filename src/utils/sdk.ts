...
import { deleteDir, ensureDir } from './paths.js';
...
const SDKs: Record<string, { fileName: string; sha256: string }> = {
  '15.4': {
    fileName: 'MacOSX-15.4.sdk.zip',
    sha256: '7bb880365a1adb9f99f011f5bec32eefc2130d3df4d0460e090a65e1387af221',
  },
  '15.2': {
    fileName: 'MacOSX-15.2.sdk.zip',
    sha256: 'bae35c93206cc14304f7416e2412dfa3838fa908f2dac76114351e47c76a09c6',
  },
  '15.1': {
...
function getSDKVersion(): 'unknown' | string {
...
function extractSDKVersion(toolchainFile: string): string | null {
...
function expectedSDKVersion(): string {
  ...
  if (isNaN(Number(version)) || !version || !SDKs[version]) {
    console.warn(
      color.warn,
...
function ensureViableXCode() {
  ...
  if (fs.existsSync(xcodeBuildExec)) {
    const result = spawnSync(xcodeBuildExec, ['-version']);
    if (result.status === 0) {
...
export function ensureSDKAndSymlink(config: EVMBaseElectronConfiguration): string {
...
export function ensureSDK(versionOverride?: string): string {
  ...
  if (versionOverride && !SDKs[versionOverride]) {
    const availableVersions = Object.keys(SDKs).join(', ');
    fatal(
      `SDK version ${versionOverride} is invalid or unsupported - please use one of the following: ${availableVersions}`,
    );
  }

  const expected = versionOverride || expectedSDKVersion();
  ...
  if (shouldEnsureSDK) {
    ...
    if (!fs.existsSync(eventualVersionedPath)) {
      ...
      try {
        const { status } = spawnSync('unzip', ['-q', '-o', SDKZip, '-d', unzipPath], {
          stdio: 'inherit',
        });
        if (status !== 0) {
        ...
        deleteDir(SDKZip);
        deleteDir(unzipPath);
        fatal(`Failed to unzip SDK zip file: ${error}`);
      }
...
function hashFile(file: string): string {
  ...
  return spawnSync('shasum', ['-a', '256', file]).stdout.toString().split(' ')[0].trim();
}
