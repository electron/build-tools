...
evmConfig.warnAboutNextConfigChange();
...
program
  ...
  .action((name) => {
    try {
      evmConfig.setCurrentConfig(name);
...
program
  ...
  .action(() => {
    ...
    const { status, error } = depotSpawnSync(evmConfig.current(), args[0], args.slice(1), {
      stdio: 'inherit',
      env: {
...
program
  ...
  .action(() => {
    ...
    const { status } = depotSpawnSync(evmConfig.current(), process.env.SHELL, [], {
      stdio: 'inherit',
      env: {
        ...process.env,
