...
import { pathKey } from '../dist/utils/path-key';
...
describe('e-show', () => {
  ...
  it('shows the current config', () => {
    ...
    const result = sandbox.eShowRunner().current().run();
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toEqual(name);
  });

  it('shows the outdir', () => {
    sandbox.eInitRunner().root(root).name(name).out(out).run();
  ...
  it('shows the exec', () => {
    sandbox.eInitRunner().root(root).name(name).out(out).run();
    const result = sandbox.eShowRunner().exec().run();
    expect(result.exitCode).toBe(0);
  ...
  it('shows the src', () => {
    const srcdir = 'base';
    sandbox.eInitRunner().root(root).name(name).out(out).run();
    const result = sandbox.eShowRunner().src(srcdir).run();
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toEqual(path.resolve(root, 'src', srcdir));
  });

  it('shows all configs', () => {
    sandbox.eInitRunner().root(`${root}1`).name(`${name}1`).out(`${out}1`).run();
    sandbox.eInitRunner().root(`${root}2`).name(`${name}2`).out(`${out}2`).run();
    const result = sandbox.eShowRunner().configs().run();
    expect(result.exitCode).toBe(0);
  ...
  it('shows env', () => {
    sandbox.eInitRunner().root(root).name(name).out(out).run();
    const result = sandbox.eShowRunner().env().run();
    expect(result.exitCode).toBe(0);
